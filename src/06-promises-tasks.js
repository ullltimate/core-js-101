 ...
 *
 */
function chainPromises(...args) {
  return args[0].reduce((acc, cur) => acc.then((accRes) => cur.then((curRes) => [...accRes, curRes])
    .catch(() => [...accRes])), Promise.resolve([]))
    .then((res) => res.reduce((acc, cur) => args[1](acc, cur)));
}
